    ...
    public partial class RoleService : IRoleService
    {
        ...
        public DataTable GetDataTable(BaseUserInfo userInfo, string systemCode)
        {
            ...
            ServiceUtil.ProcessUserCenterReadDb(userInfo, parameter, (dbHelper) =>
            {
                ...
                if (!userInfo.SystemCode.Equals("Base"))
                {
                    ...
                    DataTable dtBase = manager.GetDataTable(parameters);
        ...
        public DataTable GetDefaultDutyDT(BaseUserInfo userInfo)
        {
            ...
            ServiceUtil.ProcessUserCenterReadDb(userInfo, parameter, (dbHelper) =>
            {
                ...
                var manager = new BaseRoleManager(dbHelper, userInfo, tableName);
                List<KeyValuePair<string, object>> parameters = new List<KeyValuePair<string, object>>();
                parameters.Add(new KeyValuePair<string, object>(BaseRoleEntity.FieldCategoryCode, "DefaultDuty"));
                parameters.Add(new KeyValuePair<string, object>(BaseRoleEntity.FieldDeletionStateCode, 0));
                parameters.Add(new KeyValuePair<string, object>(BaseRoleEntity.FieldIsVisible, 1));
                dt = manager.GetDataTable(parameters, BaseRoleEntity.FieldSortCode);
        ...
        public DataTable GetUserGroup(BaseUserInfo userInfo)
        {
            ...
            ServiceUtil.ProcessUserCenterReadDb(userInfo, parameter, (dbHelper) =>
            {
                ...
                // 获得角色列表
                var manager = new BaseRoleManager(dbHelper, userInfo, tableName);
                string where = " (CreateUserId = '" + userInfo.Id + "' OR Id IN (SELECT RoleId FROM BaseUserRole WHERE (UserId = '" + userInfo.Id + "'))) AND (DeletionStateCode = 0) AND (Enabled = 1) AND (CategoryCode = 'UserGroup')";
        ...
        public DataTable Search(BaseUserInfo userInfo, string organizeId, string searchValue)
        {
            ...
            ServiceUtil.ProcessUserCenterReadDb(userInfo, parameter, (dbHelper) =>
            {
                ...
                // 获得角色列表
                var manager = new BaseRoleManager(dbHelper, userInfo, tableName);
                result = manager.Search(organizeId, searchValue);
                result.TableName = BaseRoleEntity.TableName;
            });
