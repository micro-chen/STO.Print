...
namespace DotNet.Business
{
    ...
    public partial class DbLogic
    {
        ...
        public static bool IsModifed(IDbHelper dbHelper, string tableName, string fieldName, Object fieldValue, string oldModifiedUserId, DateTime? oldModifiedOn)
        {
            ...
            var dt = dbHelper.Fill(sqlQuery, new IDbDataParameter[] { dbHelper.MakeParameter(fieldName, fieldValue)});
            result = IsModifed(dt, oldModifiedUserId, oldModifiedOn);
            return result;
        }
        #endregion

        #region private static bool IsModifed(DataTable result, string oldModifiedUserId, DateTime? oldModifiedOn) 数据是否已经被别人修改了
        /// <summary>
        /// 数据是否已经被别人修改了
        /// </summary>
        ...
        /// <param name="oldModifiedOn">最后修改时间</param>
        /// <returns>已被修改</returns>
        private static bool IsModifed(DataTable dt, string oldModifiedUserId, DateTime? oldModifiedOn)
        {
            bool result = false;
            foreach (DataRow dr in dt.Rows)
            {
                result = IsModifed(dr, oldModifiedUserId, oldModifiedOn);
            }
            return result;
        }
        #endregion

        #region public static bool IsModifed(DataRow dr, string oldModifiedUserId, DateTime? oldModifiedOn) 数据是否已经被别人修改了
        /// <summary>
        /// 数据是否已经被别人修改了
        /// </summary>
        ...
        /// <param name="oldModifiedOn">最后修改时间</param>
        /// <returns>已被修改</returns>
        public static bool IsModifed(DataRow dr, string oldModifiedUserId, DateTime? oldModifiedOn)
        {
            bool result = false;
            if ((dr[BaseBusinessLogic.FieldModifiedUserId] == DBNull.Value) || ((dr[BaseBusinessLogic.FieldModifiedOn] == DBNull.Value)))
            {
                return false;
            }
            DateTime newModifiedOn = DateTime.Parse(dr[BaseBusinessLogic.FieldModifiedOn].ToString());
            if (!dr[BaseBusinessLogic.FieldModifiedUserId].ToString().Equals(oldModifiedUserId) || newModifiedOn!=oldModifiedOn)
            {
                return true;
            }
            return result;
