...
namespace DotNet.Business
{
    ...
    public partial class BaseManager : IBaseManager
    {
        ...
        /// <param name="order">排序字段</param>
        /// <returns>数据表</returns>
        public virtual List<TModel> GetDataReaderByPage<TModel>(out int recordCount, int pageIndex, int pageSize,
            string whereClause, IDbDataParameter[] dbParameters, string order) where TModel : new()
        {
            return
                GetDataReaderByPage(out recordCount, pageIndex, pageSize, whereClause, dbParameters, order)
                    .ToList<TModel>();
        }
        ...
        public virtual IDataReader GetDataReaderByPage(out int recordCount, int pageIndex = 0, int pageSize = 20, string sortExpression = null, string sortDire = null, string tableName = null, string conditional = null, IDbDataParameter[] dbParameters = null, string selectField = null)
        {
            ...
            if (tableName.ToUpper().IndexOf("SELECT") >= 0 || DbHelper.CurrentDbType == DotNet.Utilities.CurrentDbType.MySql)
            {
                ...
                return DbLogic.GetDataReaderByPage(DbHelper, recordCount, pageIndex, pageSize, tableName, dbParameters, sortExpression, sortDire);
            }
            // 这个是调用存储过程的方法
            return DbLogic.GetDataReaderByPage(DbHelper, out recordCount, pageIndex, pageSize, sortExpression, sortDire, tableName, conditional, selectField);
        }
        ...
        /// <param name="selectField">选择哪些字段</param>
        /// <returns>数据表</returns>
        public virtual List<TModel> GetDataReaderByPage<TModel>(out int recordCount, int pageIndex = 0, int pageSize = 20,
            string sortExpression = null, string sortDire = null, string tableName = null, string conditional = null,
            IDbDataParameter[] dbParameters = null, string selectField = null) where TModel : new()
        {
            return
                GetDataReaderByPage(out recordCount, pageIndex, pageSize, sortExpression, sortDire, tableName,
                    conditional, dbParameters, selectField).ToList<TModel>();
        }
    }
}