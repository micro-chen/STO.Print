...
namespace DotNet.Business
{
    ...
    public partial class BaseManager : IBaseManager
    {
        ...
        {
            return this.SetProperty(new KeyValuePair<string, object>(this.PrimaryKey, id), parameter);
        }
        ...
        {
            return this.SetProperty(this.PrimaryKey, ids, parameter);
        }
        ...
        public virtual int SetProperty(string name, object[] values, KeyValuePair<string, object> parameter)
        {
            ...
            if (values == null)
            {
                result += this.SetProperty(new KeyValuePair<string, object>(name, string.Empty), parameter);
            }
            else
            {
                for (int i = 0; i < values.Length; i++)
                {
                    result += this.SetProperty(new KeyValuePair<string, object>(name, values[i]), parameter);
                }
            }
            return result;
        }
