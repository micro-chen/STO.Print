    ...
    public partial class BaseNewsManager : BaseManager, IBaseManager
    {
        ...
        public override int SetDeleted(object[] ids, bool enabled = false, bool modifiedUser = false)
        {
            ...
            for (int i = 0; i < ids.Length; i++)
            {
                ...
                // {
                result += this.SetDeleted(ids[i], enabled, modifiedUser);
                //}
            }
            return result;
        }
        ...
        public string ShowNews(string id)
        {
            ...
            try
            {
                using (IDataReader dataReader = DbHelper.ExecuteReader(sqlQuery, new IDbDataParameter[] { DbHelper.MakeParameter(BaseNewsEntity.FieldId, id) }))
                {
                    if (dataReader.Read())
                    {
        ...
        {
            // 检查是否已经存在
            string result = this.GetId(new KeyValuePair<string, object>(BaseNewsEntity.FieldFolderId, folderId), new KeyValuePair<string, object>(BaseNewsEntity.FieldTitle, title));
            if (!String.IsNullOrEmpty(result))
            {
                // 更新数据
                this.UpdateFile(result, title, contents);
            }
            else
            {
                // 添加数据
                ...
                result = this.AddObject(fileEntity);
            }
            return result;
        }
        ...
        {
            statusCode = string.Empty;
            int result = this.UpdateObject(fileEntity);
            if (result > 0)
            {
        ...
        public DataTable Search(string departmetId, string userId, string categoryCode, string searchValue, bool? enabled, bool? deletionStateCode)
        {
            ...
            if (enabled != null)
            {
                ...
                sqlQuery += string.Format(" AND {0} = {1}", BaseNewsEntity.FieldEnabled, isEnabled);
            }
            if (!string.IsNullOrEmpty(departmetId))
            {
                sqlQuery += string.Format(" AND {0} = '{1}' ", BaseNewsEntity.FieldDepartmentId, departmetId);
            }
            // 三、我们认为 userId 这个查询条件是安全，不是人为输入的参数，所以直接进行了SQL语句拼接
            if (!String.IsNullOrEmpty(userId))
            {
                sqlQuery += string.Format(" AND {0} = {1}", BaseNewsEntity.FieldCreateUserId, userId);
            }
            /*
            ...
            */
            ...
            {
                sqlQuery += string.Format(" AND {0} = '{1}'", BaseNewsEntity.FieldCategoryCode, categoryCode);
            }
            ...
            if (!String.IsNullOrEmpty(searchValue))
            {
                ...
                sqlQuery += string.Format(" AND ({0} LIKE {1}", BaseNewsEntity.FieldTitle, DbHelper.GetParameter(BaseNewsEntity.FieldTitle));
                sqlQuery += string.Format(" OR {0} LIKE {1}", BaseNewsEntity.FieldDepartmentName, DbHelper.GetParameter(BaseNewsEntity.FieldDepartmentName));
                sqlQuery += string.Format(" OR {0} LIKE {1}", BaseNewsEntity.FieldCategoryCode, DbHelper.GetParameter(BaseNewsEntity.FieldCategoryCode));
                sqlQuery += string.Format(" OR {0} LIKE {1}", BaseNewsEntity.FieldCreateBy, DbHelper.GetParameter(BaseNewsEntity.FieldCreateBy));
                sqlQuery += string.Format(" OR {0} LIKE {1}", BaseNewsEntity.FieldContents, DbHelper.GetParameter(BaseNewsEntity.FieldContents));
                sqlQuery += string.Format(" OR {0} LIKE {1})", BaseNewsEntity.FieldDescription, DbHelper.GetParameter(BaseNewsEntity.FieldDescription));
