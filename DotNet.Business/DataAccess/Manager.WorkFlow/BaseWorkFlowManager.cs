...
namespace DotNet.Business
{
    ...
    public partial class BaseWorkFlowCurrentManager : BaseManager, IBaseManager
    {
        ...
        public DataTable GetAllWaitForAudit()
        {
            ...
            var dt = DbHelper.Fill(sqlQuery, DbHelper.MakeParameters(names, values));
            return dt;
        }
        #endregion
        ...
        {
            string[] roleIds = new BaseUserManager(UserInfo).GetRoleIds(this.UserInfo.Id);
        ...
        public DataTable GetShareBillDT(string categoryFullName = null, string searchValue = null)
        {
            ...
            sqlQuery += @")                     
                       ...
                       AND (BaseWorkFlowActivity.ProcessId = BaseWorkFlowCurrent.ProcessId) ";
            List<IDbDataParameter> dbParameters = new List<IDbDataParameter>();
        ...
        /// <param name="searchValue">查询内容</param>
        /// <returns>数据表</returns>
        public DataTable GetMonitorDTByPage(int pageSize, int pageIndex, out int recordCount, string categoryCode = null, string searchValue = null, bool unfinishedOnly = true)
        {
            string whereClause = string.Empty;
            whereClause = BaseWorkFlowCurrentEntity.FieldDeletionStateCode + " = 0 ";
            if (unfinishedOnly)
            {
                whereClause += " AND " + BaseWorkFlowCurrentEntity.FieldEnabled + " = 0 ";
            }
            if (!string.IsNullOrEmpty(categoryCode))
            {
                whereClause += " AND ProcessId IN (SELECT Id FROM BaseWorkFlowProcess WHERE (CategoryCode = '" + categoryCode + "')) ";
            }
            searchValue = StringUtil.GetSearchString(searchValue);
            if (!String.IsNullOrEmpty(searchValue))
            {
                searchValue = searchValue.Trim();
                whereClause += " AND (" + BaseWorkFlowCurrentEntity.FieldObjectFullName + " LIKE '" + searchValue + "'";
                whereClause += " OR " + BaseWorkFlowCurrentEntity.FieldCreateBy + " LIKE '" + searchValue + "'";
                whereClause += " OR " + BaseWorkFlowCurrentEntity.FieldAuditUserRealName + " LIKE '" + searchValue + "'";
                whereClause += " OR " + BaseWorkFlowCurrentEntity.FieldAuditIdea + " LIKE '" + searchValue + "'";
            ...
            return this.GetDataTableByPage(out recordCount, pageIndex, pageSize, whereClause, null, BaseWorkFlowCurrentEntity.FieldSortCode);
        }
        ...
        public DataTable GetUserAllBill(string categoryId, string billCategoryCode = null, string searchValue = null)
        {
            ...
            {
                BaseWorkFlowBillTemplateManager templateManager = new BaseWorkFlowBillTemplateManager(this.DbHelper, this.UserInfo);
                var dt = templateManager.Search(string.Empty, categoryId, string.Empty, null, false);
                string categoryCodes = BaseBusinessLogic.FieldToList(dt, BaseWorkFlowBillTemplateEntity.FieldCode);
                if (!string.IsNullOrEmpty(categoryCodes))
                {
                    sqlQuery += " AND (BaseWorkFlowCurrent.CategoryCode IN (" + categoryCodes + ")) ";
