...
namespace DotNet.ScreenShot
{
    public partial class CaptureImageTool : Form
    {
        ...
        /// 遮罩层颜色
        /// </summary>
        private static readonly SolidBrush Mask = new SolidBrush(Color.FromArgb(100, 0, 0, 0));
        /// <summary>
        /// 原始屏幕图
        /// </summary>
        ...
        private const string ToolTipStartCapture = "按住左键不放选择截图区域";
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            oldg.DrawImage(this._screenImage, SelectImageRect, SelectImageRect, GraphicsUnit.Pixel);
        ...
        private void DrawLastImage()
        {
                ...
                Width, Height, PixelFormat.Format32bppArgb))
            {
                using (Graphics allGraphics = Graphics.FromImage(allBmp))
                {
                    ...
                    //使用原始图片获取截图（涂剑凯修改）
                    allGraphics.DrawImage(_screenImage, Point.Empty);
                    //使用窗体背景图片截图（原版）
        ...
        private void DrawToolsControlButtonSaveClick(object sender, EventArgs e)
        {
            if (SelectedImage)
            {
                ...
                saveFileDialog.FileName = "Y" + DateTime.Now.ToString("yyyyMMddHHmmss") + "." + saveFileDialog.DefaultExt;
        ...
        private void Init()
        {
            ...
            //保留当前屏
            _screenImage = GetDestopImage();
            //复制当前屏
            Image BackScreen = new Bitmap(_screenImage);
