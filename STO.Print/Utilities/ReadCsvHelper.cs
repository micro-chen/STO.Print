...
namespace STO.Print.Utilities
{
    ...
    public class ReadCsvHelper
    {
        ...
        public ReadCsvHelper(string filepath)
        {
        ...
        public ReadCsvHelper(string filepath, int headIndex)
        {
            SetFilepath(filepath);
            SetHeadindex(headIndex);
        ...
        public ReadCsvHelper(string filepath, int headIndex, int dataIndex)
        {
            SetFilepath(filepath);
            SetHeadindex(headIndex);
            SetDataindex(dataIndex);
        ...
        private void ReadCsv2(string filepath, int headIndex, int dataIndex)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show(string.Format("数据读取失败!失败原因：{0}", err.Message), AppMessage.MSG0000, System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information);
            }
        }
        ...
        private void FillData(string dataline)
        {
            ...
            for (int index = 0; index < dataline.Length; index++)
            {
                if (dataline[index] == '"')
                {
                    ...
                    {
                        drow[_resoultTable.Columns[columindex]] = dataline.Substring(lastdouhao).Trim('"').Replace("\"\"", "\"").Replace("=", "").Replace("\"", "");
                        columindex++;
                    }
                }
                if (dataline[index] == ',' && countyinhao % 2 == 0)
                {
                    drow[_resoultTable.Columns[columindex]] = dataline.Substring(lastdouhao, index - lastdouhao).Trim('"').Replace("\"\"", "\"").Replace("=", "").Replace("\"", "");
                    columindex++;
                    countyinhao = 0;
                    lastdouhao = index + 1;
                    if (index == dataline.Length - 1)
                    {
                        drow[_resoultTable.Columns[columindex]] = dataline.Substring(index + 1).Trim('"').Replace("\"\"", "\"").Replace("=", "").Replace("\"", "");
                        columindex++;
                    }
                }
