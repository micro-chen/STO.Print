...
namespace ZTO.Print
{
    ...
    public partial class FrmImportReceiveManExcel : BaseForm
    {
        private readonly BaseAreaManager _areaManager = new BaseAreaManager(BillPrintHelper.DbHelper);

        public FrmImportReceiveManExcel()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                if (chooseDt != null && chooseDt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in chooseDt.Rows)
                    {
                        ...
                        var userEntity = new ZtoUserEntity { Realname = BaseBusinessLogic.ConvertToString(dr[8]) };
